...
class Dokumen_final_model extends Model {
    ...
    public function mget_final($request, $table, $primaryKey, $columns)

    {

        $result = $this->mgetfinal($request, $table, $primaryKey, $columns);
	...
	public function getfiles($request, $table, $primaryKey, $columns, $id, $y)
	{
		...
		$result = $this->simple_file_final($request, $table, $primaryKey, $columns, $id, $y);
    ...
    {
        $result = $this->query("SELECT a.autocode, a.`nama_kondisi`, IF(b.autocode IS NULL, '', 'selected') AS pselct FROM ref_kondisi  a  LEFT JOIN ( SELECT  autono, autocode, kondisi_media FROM tbl_dokumen WHERE autono = $id) b ON b.`kondisi_media` = a.`autocode` ORDER BY a.nama_kondisi ASC");
    ...
    {
    	$result = $this->query("SELECT autocode, nama_personel FROM ref_personel ORDER BY nama_personel ASC");
    ...
    {
    	$result = $this->query("SELECT a.autocode, a.`nama_personel`, IF(b.kd_personel IS NULL, '', 'selected') AS pselct FROM ref_personel a   LEFT JOIN ( SELECT  parent_id, parent_autocode, kd_personel FROM tbl_dokumen_personel WHERE parent_id = $id) b ON b.`kd_personel` = a.`autocode`");
    ...
    {
        $result = $this->query("SELECT a.autocode, a.`nama_personel`, IF(b.kd_personel IS NULL, '', 'selected') AS pselct FROM ref_fotografer a   LEFT JOIN ( SELECT  parent_id, parent_autocode, kd_personel FROM tbl_dokumen_fotografer WHERE parent_id = $id) b ON b.`kd_personel` = a.`autocode`");
    ...
    {
    	$result = $this->query("SELECT a.autocode, a.`nama_satker`, IF(b.kd_satker IS NULL, '', 'selected') AS pselct FROM ref_satker  a  LEFT JOIN ( SELECT  parent_id, parent_autocode, kd_satker FROM tbl_dokumen_satker WHERE parent_id = $id) b ON b.`kd_satker` = a.`autocode`");
    ...
    {
    	$result = $this->query("SELECT a.autocode, a.`nama_kategori`, IF(b.kd_kategori IS NULL, '', 'selected') AS pselct FROM ref_kategori  a LEFT JOIN ( SELECT  parent_id, parent_autocode, kd_kategori FROM tbl_dokumen_kategori WHERE parent_id = $id) b ON b.`kd_kategori` = a.`autocode`");
    ...
    {
    	$result = $this->query("SELECT a.autocode, a.`nama_media`, IF(b.autocode IS NULL, '', 'selected') AS pselct FROM ref_media  a  LEFT JOIN ( SELECT  autono, autocode, jenis_media FROM tbl_dokumen WHERE autono = $id) b ON b.`jenis_media` = a.`autocode` ORDER BY a.autono ASC");

    	return $result;
    }

    public function get_kategorivideo()
    {
        $result = $this->query("SELECT autocode, nama_kategori FROM ref_kategori_video ORDER BY autono ASC");

        return $result;
    }

    public function get_kategorivideoedit($id)
    {
        $result = $this->query("SELECT a.autocode, a.`nama_kategori`, IF(b.autocode IS NULL, '', 'selected') AS pselct FROM ref_kategori_video  a  LEFT JOIN ( SELECT  autono, autocode, kategori_video FROM tbl_dokumen WHERE autono = $id) b ON b.`kategori_video` = a.`autocode`  ORDER BY a.autono ASC");
    ...
    public function simple_file_final ( $request, $table, $primaryKey, $columns, $id, $key)

    {
        ...
        $sWhere = "WHERE `$primaryKey` = '$id' AND `subdir` = 'Final'";
    ...
    public function mgetfinal ( $request, $table, $primaryKey, $columns)

    {
        ...
        $where = self::filterss( $request, $columns, $bindings, true );

        $sWhere = "WHERE final = 1";

        $sJoin  = "LEFT JOIN ( SELECT parent_id, nama_file FROM vt_files WHERE tipe_file NOT LIKE '%image%' AND tipe_file  NOT LIKE '%pdf%' GROUP BY parent_id) AS vt_files ON tbl_dokumen.`autono` = vt_files.`parent_id`";
    ...
    public function filterss ( $request, $columns, &$bindings )
