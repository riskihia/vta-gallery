...
class Dokumen_doc_model extends Model {
    ...
    public function mget_doc($request, $table, $primaryKey, $columns)

    {

        $result = $this->mgetdoc($request, $table, $primaryKey, $columns);
    ...
    {
    	$result = $this->query("SELECT autocode, nama_media FROM ref_media WHERE direktori = 'Dokumen' ORDER BY nama_media ASC");
    ...
    {
    	$result = $this->query("SELECT a.autocode, a.`nama_media`, IF(b.autocode IS NULL, '', 'selected') AS pselct FROM ref_media  a  LEFT JOIN ( SELECT  autono, autocode, jenis_media FROM tbl_dokumen WHERE autono = $id) b ON b.`jenis_media` = a.`autocode` WHERE direktori = 'Dokumen' ORDER BY a.nama_media ASC");
    ...
    public function mgetdoc ( $request, $table, $primaryKey, $columns)

    {
        ...
        $sJoin  = "LEFT JOIN ( SELECT parent_id, nama_file FROM vt_files WHERE tipe_file LIKE '%pdf%' GROUP BY parent_id) AS vt_files ON tbl_dokumen.`autono` = vt_files.`parent_id`";
