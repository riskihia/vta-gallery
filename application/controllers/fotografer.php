...
class Fotografer extends Controller {
	...
	private $table      = "ref_fotografer";
	private $primaryKey = "autono";
	private $model      = "Fotografer_model"; # please write with no space
	private $menu       = "Reference";
	private $title      = "Fotografer";
	private $curl       = BASE_URL."fotografer/";
	...
	function index()
	{
		...
		$data['curl']		 = $this->curl;
		$template            = $this->loadView('fotografer_view');
	...
	public function add()
	{
		...
		$data['action']      = 'Add';
		$data['curl']		 = $this->curl;
		$template            = $this->loadView('fotografer_add');
		$template->set('data', $data);
		$template->render();
	}
	...
	public function edit($x)
	{
		...
		$template            = $this->loadView('fotografer_edit');
		$template->set('data', $data);
		$template->render();
	}
	...
	public function save()
	{
		...
		$data['autocode']      = $model->autocode($this->table, "FG");	
		$result                = $model->msave($this->table, $data, $this->title);
		$this->redirect('fotografer');
	}
