...
class Jenis_media extends Controller {



	private $table      = "ref_media";

	private $primaryKey = "autono";

	private $model      = "Jenis_media_model";

	private $menu       = "Referensi";

	private $title      = "Jenis Media";

	private $curl       = BASE_URL."jenis_media";
...

	{
		...
		$template            = $this->loadView('jenis_media_view');
...

	{
		...
		$columns = array(
			...
			array( 'db' => 'nama_media',  'dt' => 1 ),array( 'db' => 'keterangan',  'dt' => 2 ),
...

	{
		...
		$template            = $this->loadView('jenis_media_add');
...

	{
		...
		$template            = $this->loadView('jenis_media_edit');
...

	{
		...
		$data['nama_media'] = htmlspecialchars($_REQUEST['nama_media']) ;
		$data['keterangan'] = htmlspecialchars($_REQUEST['keterangan']) ;
		$direktori          = explode(" ", trim($data['nama_media']));
		$data['direktori']  = $direktori[0] ;


		$data['autocode']     = $model->autocode($this->table, "MED-");	

		$result               = $model->msave($this->table, $data, $this->title);

		if($x){

			$this->redirect('jenis_media/detail/'.$x);

		} else {

			$this->redirect('jenis_media');
...

	{
		...
		if($child){
			...
			$this->redirect('jenis_media/detail/'.$child);
