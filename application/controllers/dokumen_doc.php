...
class Dokumen_doc extends Controller {
	...
	private $model      = "Dokumen_doc_model";

	private $menu       = "Transaksi";

	private $title      = "Manajemen Dokumen";

	private $curl       = BASE_URL."dokumen_doc";
...

	{
		...
		$template            = $this->loadView('dokumen_doc_view');
...

	{
		...
		if($x){
			...
			$result  = $model->mget_doc($request, $this->table, $this->primaryKey, $columns);
...

	{
		...
		$template            = $this->loadView('dokumen_doc_add');
...

	{
		...
		$template              = $this->loadView('dokumen_doc_edit');
...

	{
		...
		if($x){
			...
			$this->redirect('dokumen_doc/detail/'.$x);

		} else {

			$this->redirect('dokumen_doc');
...

	{
		...
		if($child){
			...
			$this->redirect('dokumen_doc/detail/'.$child);
	...
	public function uploaddoc($x = null)

	{
		...
		$files1['dir'] 			 = "dokumen";
		$files1['subdir'] 		 = "Image";
		if(!empty($_FILES['file_doc']['name'][0])) {
		    $file_ary1 = $model->reArrayFiles($_FILES['file_doc']);
		    foreach ($file_ary1 as $file1) {
				$files1['kode_parent'] = $autocode;
				$files1['parent_id']   = $id;
            	$files1['nama_file']   = $file1['name'];
		...
		# Upload file
		if(isset($_FILES['file_doc'])){ $model->uploads('dokumen', $_FILES['file_doc'], $autocode, $files1['subdir']); }
	...
	public function zipdownx()
	{
		...
		$zip_name = "document_download_".time().".zip";
