...
class Dokumen_final extends Controller {
	...
	private $model      = "Dokumen_final_model";

	private $menu       = "Transaksi";

	private $title      = "Video/Foto Final";

	private $curl       = BASE_URL."dokumen_final";
...

	{
		...
		$template            = $this->loadView('dokumen_final_view');
...

	{
		...
		$columns = array(
			...
			array( 'db' => 'narasi',  'dt' => 6 , 'formatter' => function( $d, $row ) { return $this->base64url_encode($d); } ),
			array( 'db' => 'complete',  'dt' => 7 ),
			array( 'db' => 'created_by',   'dt' => 8 ),
			array( 'db' => 'keterangan',   'dt' => 9 , 'formatter' => function( $d, $row ) { return $this->base64url_encode($d); } ),
		...
		if($x){
			...
			$result  = $model->mget_final($request, $this->table, $this->primaryKey, $columns);
...

	{
		...
		$data['kategori_video'] = $model->get_kategorivideo();
		...
		$template               = $this->loadView('dokumen_final_add');
...

	{
		...
		$data['kategori_video'] = $model->get_kategorivideoedit($id);
		...
		$template               = $this->loadView('dokumen_final_edit');
...

	{
		...
		$data['kategori_video']= $model->escapeString($_REQUEST['kategori_video']) ;
		...
		# Upload file
		if(isset($_FILES['file_dokumen'])){ $model->uploads('final', $_FILES['file_dokumen'], $data['autocode'], $files1['subdir']); }

		if($x){

			$this->redirect('dokumen_final/detail/'.$x);

		} else {

			$this->redirect('dokumen_final');
...

	{
		...
		# Upload file
		if(isset($_FILES['file_dokumen'])){ $model->uploads('final', $_FILES['file_dokumen'], $autocode, $files1['subdir']); }

		if($child){

			$this->redirect('dokumen_final/detail/'.$child);
	...
	public function uploadvideo($x = null)

	{
		...
		# Upload file
		if(isset($_FILES['file_video'])){ $model->uploads('final', $_FILES['file_video'], $autocode, $files1['subdir']); }
	...
	public function zipdown()
	{
		...
		$datafile =  $model->query("SELECT GROUP_CONCAT(autono SEPARATOR '-') as te, CONCAT('static/files/bahan/',dir,'/', kode_parent,'/',subdir,'/',nama_file ) AS direktori FROM vt_files WHERE autono IN ($autono)");
	...
	public function zipdownx()
	{
		...
		$zip_name = "video_download_".time().".zip";
		$datafile =  $model->query("SELECT CONCAT('static/files/bahan/',dir,'/', kode_parent,'/',subdir,'/',nama_file ) AS direktori FROM vt_files WHERE autono IN ($autono)");
		$files    = array();
