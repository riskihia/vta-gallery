...
class Dokumen_lisjarah extends Controller {



	private $table      = "tbl_dokumen_lisjarah";

	private $primaryKey = "autono";

	private $model      = "Dokumen_lisjarah_model";

	private $menu       = "Transaksi";

	private $title      = "Data Lisjarah";

	private $curl       = BASE_URL."dokumen_lisjarah";
...

	{
		...
		$template            = $this->loadView('dokumen_lisjarah_view');
...

	{
		...
		if($x){
			...
			$result  = $model->mget_video($request, $this->table, $this->primaryKey, $columns);
	...
	function getfile($x, $y)
	{
		...
		$model   = $this->loadModel($this->model);
		$result  = $model->getfiles($request, $sTable, $primaryKey, $columns, $id, $y, $this->table);
...

	{
		...
		$template            = $this->loadView('dokumen_lisjarah_add');
...

	{
		...
		$template              = $this->loadView('dokumen_lisjarah_edit');
...

	{
		...
		if($x){
			...
			$this->redirect('dokumen_lisjarah/detail/'.$x);

		} else {

			$this->redirect('dokumen_lisjarah');
...

	{
		...
		if($child){
			...
			$this->redirect('dokumen_lisjarah/detail/'.$child);
...

	{
		...
		# Upload file
		if(isset($_FILES['file_video'])){ $model->uploads('dokumen', $_FILES['file_video'], $autocode, $files1['subdir']); }
	...
	public function zipdown()
	{
		...
		$datafile =  $model->query("SELECT GROUP_CONCAT(autono SEPARATOR '-') as te, CONCAT('static/files/bahan/dokumen/', kode_parent,'/',subdir,'/',nama_file ) AS direktori FROM vt_files WHERE autono IN ($autono)");
	...
	public function zipdownx()
	{
		...
		$datafile =  $model->query("SELECT CONCAT('static/files/bahan/dokumen/', kode_parent,'/',subdir,'/',nama_file ) AS direktori FROM vt_files WHERE autono IN ($autono)");
		$files    = array();
