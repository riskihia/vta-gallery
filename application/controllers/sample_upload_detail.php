...
class Sample_upload_detail extends Controller {

	private $table      = "sample_upload_tbl_detail";
	private $primaryKey = "autono";
	private $model      = "Sample_upload_detail_model";
	private $menu       = "Front Office";
	private $title      = "sample upload Detail";
	private $curl       = BASE_URL."sample_upload_detail";
	...
	function index()
	{
		...
		$data['curl']	     = $this->curl;
		$template            = $this->loadView('sample_upload_detail_view');
	...
	public function add($x = null)
	{
		...
		$data['curl']	     = $this->curl;
		$data['encode']	     = $x;
		$template            = $this->loadView('sample_upload_detail_add');
		$template->set('data', $data);
		$template->render();
	}
	...
	public function edit($x)
	{
		...
		$template            = $this->loadView('sample_upload_detail_edit');
		$template->set('data', $data);
		$template->render();
	}
	...
	public function save($x = null)
	{
		...
		$data['nama_lagi'] = htmlspecialchars($_REQUEST['nama_lagi']) ;

		$data['autocode']     = $model->autocode($this->table, "SU-");	
		$result               = $model->msave($this->table, $data, $this->title);
		if($x){
			$this->redirect('sample_upload_detail/detail/'.$x);
		} else {
			$this->redirect('sample_upload_detail');
		}
	}
	...
	public function update($x)
	{
		...
		if($child){
			$this->redirect('sample_upload_detail/detail/'.$child);
		} else {
			$this->redirect('sample_upload_detail');
		}
	}
