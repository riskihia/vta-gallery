...
class Laporan_upload_dokumen_jenis extends Controller {

	private $table       = "tbl_dokumen";
	private $primaryKey  = "autono";
	private $model       = "Laporan_model"; # please write with no space
	private $title       = "Laporan Upload Dokumen per Jenis Media";
	private $menu        = "Laporan";
	private $curl        = BASE_URL."laporan_upload_dokumen_jenis";
...
LEFT JOIN ref_kondisi c ON a.`kondisi_media` = c.`autocode`";
	...
	function index()
	{
		...
		$data['curl']        = $this->curl;
        $data['jenis']       = $model->getjenis();
		$template            = $this->loadView('laporan_upload_dokumen_jenis');
...

	{
		...
		$model   = $this->loadModel($this->model);
		$result  = $model->mget_jenis($request, $this->table, $this->primaryKey, $columns, $this->sQuery, 'jenis_media', $x);
	...
	function print_report($x)
     {
        ...
        if($x == 'all'){
            ...
            $sWhere   = "WHERE a.jenis_media = '".$x."'";
            $nama_media = $model->getvalue("SELECT nama_media FROM ref_media WHERE autocode='$x'");
        }
       ...
       // $pdf->Image(BASE_URL.'static/images/mabesad.jpg', 140, 3, 10);
        ...
        $pdf->Cell(160,7,'LAPORAN UPLOAD DOKUMEN '.strtoupper($nama_media[0]).'',0,1,'C');
	    $pdf->Cell(55);
