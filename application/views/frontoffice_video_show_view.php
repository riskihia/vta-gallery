	...
	<div class="page-header">
		<div class="page-header-content">
			<div class="page-title">
				<h4><a href="#" onclick="javascript:window.history.back();" title="Kembali"><i class="icon-arrow-left52 position-left"></i></a> <span class="text-semibold">Show</span> - Video</h4>

				<ul class="breadcrumb breadcrumb-caret position-right">
					<li><a href="<?php BASE_URL."frontoffice/"; ?>">Home</a></li>
					<li><a href="<?php BASE_URL."frontoffice/gallery_video"; ?>">Front Office</a></li>
		...
		<div class="page-content">
			...
			<div class="content-wrapper">
				...
				<?php $c = new Controller(); $m = new Model(); foreach ($data['pencarian']['aadata'] as $key => $value) { 
					...
					<div class="col-lg-8 col-sm-4">
						<div class="thumbnail">
							<div class="thumb">
								<video width="100%" height="500px" controls autoplay controlsList="nodownload" style="outline: none;border: none;background: #090909"><source src="<?php echo $videofile."#t=20"; ?>" type="video/mp4" preload="auto"></video>
							</div>

							<div class="caption">
								<h1 class="no-margin"><a href="<?php echo BASE_URL."front_office/show_video/".$idx;?>" class="text-default" title="<?php echo $value['nama_kegiatan'];?>"><?php echo $string ?></a> <a href="#" class="text-muted">&nbsp;</a> <div class="text-right" style="margin-top:-35px"><button type="button" class="btn btn-default btn-xs heading-btn" data-toggle="modal" data-target="#modal_form"><i class="icon-file-download2 position-left"></i> Download</button></div></h1>
                            	<span class="help-block text-grey text-size-large"><i class="icon-calendar3 pull-left"></i>&nbsp;  <?php echo $tgl; ?></span>

                            	<p><?php echo $value['narasi'] ?></p>
							</div>
						</div>
					</div>
					<?php } ?>

					<?php foreach ($data['relasi'] as $key => $rel) { $ids = $c->base64url_encode($rel['autono']); 
                            $string_rel = $rel['nama_kegiatan']; 
            				if (strlen($string_rel) > 28) {
                         		 $string_relCut = substr($string_rel, 0, 28);
                         		 $endPoint  = strrpos($string_relCut, ' ');
                         		 $string_rel    = $endPoint? substr($string_relCut, 0, $endPoint) : substr($string_relCut, 0);
                         		 $string_rel   .= "...";
                      		}
                                                                     
                            if($rel['tanggal'] != ''){
                                	$tgli = " ".$m->format_tanggal($rel['tanggal']);
                                } else {
                                	$tgli = 'Tanggal belum ada';
                                }

                                if($rel['structured'] == 0){
			                    	
									$relvideofile = BASE_URL."static/files/bahan/".$rel['dir']."/".$rel['kode_parent']."/".$rel['subdir']."/".$rel['nama_file'];
			                    } else {
			                    	$relvideofile = BASE_URL."static/files/bahan/".$rel['dir']."/".$rel['subdir']."/".$rel['nama_file'];
			                    }
            ...
            ?>
					...
					<div class="col-lg-2 col-sm-1">
						<div class="thumbnail">
							<div class="thumb">
								<video width="100%" height="120px" controls controlsList="nodownload" style="outline: none;border: none;background: #090909"><source src="<?php echo $relvideofile."#t=20"; ?>" type="video/mp4" preload="auto"></video>
							</div>
							<div class="help-block">
								<a href="<?php echo BASE_URL."frontoffice/show_video/".$ids;?>" class="text-default text-semibold text-size-large" title="<?php echo $rel['nama_kegiatan'];?>"><?php echo $string_rel; ?></a>  
                            	<span class="help-block text-grey text-size-large"><i class="icon-calendar3 pull-left"></i>&nbsp;  <?php echo $tgli; ?></span>
 							</div>
						</div>
					</div>	
