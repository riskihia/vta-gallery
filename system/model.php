...
class Model {
		    ...
		    foreach ($file_ary as $file) {
		        ...
		        if(!empty($namefile)){
		            ...
		            if(!file_exists($storagefile)){
		                ...
		                $createthumb = exec("ffmpeg -ss 00:00:20 -i ".$storagename." -vf scale=800:-1 -vframes 1 ".$storagefile."/".$ext[0].".jpg"); 
		                ...
		                $createthumb = exec("ffmpeg -ss 00:00:1 -i ".$storagefile."/".$ext[0].".mp4"." -vframes 1 ".$storagefile."/".$ext[0].".jpg"); 
		        ...
		        if(!empty($namefile)){
		            ...
		            if(!file_exists($storagefile)){
		                ...
		                if(strtolower($ext[1]) == "mp4"){
		                	if(!file_exists($filename_mp4)){
		                		//exec("ffmpeg -i ".$storagename." -s 480x320 ".$filename_mp4);
		                		exec("ffmpeg -ss 00:00:10 -i ".$storagename." -vf scale=800:-1 -vframes 1 ".$storagefile."/".$ext[0].".jpg"); 
		                	}
		                }  else {
		                	exec("ffmpeg -ss 00:00:20 -i ".$storagename." -vf scale=800:-1 -vframes 1 ".$storagefile."/".$ext[0].".jpg"); 
		                }
    ...
    //       	$html.= "\n$tab<li class=\"navigation-header\"><span>".$v->title."</span> <i class=\"icon-menu\" title=\"".$v->title."\"></i></li>\n"; 

    //     	$child = $this->get_submenu($data, $v->id);
    ...
    public function get_submenu($data, $parent = 0) 
    {
      static $i = 1;
      $tab = str_repeat("\t\t\t\t", $i);
      if (isset($data[$parent])) {
